...
describe ProbeCreator, ".new with nothing passed in" do
  ...
  it "initializes proxy with Object" do
    proxy = RR::ProbeCreator.new(@space)
    class << proxy
      attr_reader :subject
    end
...
describe ProbeCreator, ".new with one thing passed in" do
  ...
  it "initializes proxy with passed in object" do
    subject = Object.new
    proxy = RR::ProbeCreator.new(@space, subject)
    class << proxy
      attr_reader :subject
    end
...
describe ProbeCreator, ".new with two things passed in" do
  ...
  it "raises Argument error" do
    proc {RR::ProbeCreator.new(@space, nil, nil)}.should raise_error(ArgumentError, "wrong number of arguments (2 for 1)")
  end
end
